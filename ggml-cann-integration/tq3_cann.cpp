...
static void dequant_tq3_4s_block(const uint8_t blk[16], float * out) {
    float sc[4];
...
static void dequant_tq3_1s_block(const uint8_t blk[16], float * out) {
    ...
    if (src->type == GGML_TYPE_TQ3_4S) {
        for (int64_t r = 0; r < rows; r++)
            for (int64_t b = 0; b < nb; b++)
                dequant_tq3_4s_block(data + (r * nb + b) * 16,
                                     dst + r * cols + b * 32);
    } else {
        for (int64_t r = 0; r < rows; r++)
            for (int64_t b = 0; b < nb; b++)
                dequant_tq3_1s_block(data + (r * nb + b) * 16,
                                     dst + r * cols + b * 32);
    }
}
